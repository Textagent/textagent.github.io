/**
 * AI Worker — GLM-OCR (1.5B) (THUDM) — Vision OCR
 *
 * Converts document/image text using THUDM's GLM-OCR vision-language
 * model via Transformers.js (AutoModelForVision2Seq + AutoProcessor).
 * Supports WebGPU acceleration. Uses q4f16 quantization (~650 MB).
 *
 * Message interface:
...
// Model config
let MODEL_ID = "textagent/GLM-OCR-ONNX";
let MODEL_LABEL = "GLM-OCR (1.5B)";
...
async function loadModel() {
    try {
        ...
        self.postMessage({
            type: "error",
            message: `Failed to load GLM-OCR model: ${error.message}`,
        });
    }
}
 ...
 * @param {string} options.outputFormat - 'markdown', 'text', or 'html'
 * @param {boolean} options.doImageSplitting - Split image into patches for more accuracy
 * @param {string} options.messageId
 */
async function processDocument({ imageData, outputFormat = 'text', doImageSplitting = true, mimeType = 'image/png', messageId }) {
    if (!model || !processor) {
        self.postMessage({
            type: "error",
    ...
    try {
        ...
        let promptText = "Extract all text from this image.";
...
self.addEventListener("message", async (event) => {
    ...
    switch (type) {
        ...
        case "generate": {
            ...
            // Map OCR card mode to GLM-OCR outputFormat
            let outputFormat = 'text';
            if (ocrMode === 'text') outputFormat = 'text';
            else if (ocrMode === 'svg') outputFormat = 'markdown'; // GLM-OCR doesn't do SVG — use markdown
            ...
            if (imageAtt) {
                ...
                self.postMessage({
                    type: "error",
                    message: "GLM-OCR requires a document image. Please attach an image.",
                    messageId,
                });
            }
            break;
